...
<?php if ( have_posts() ) : ?>
	...
	<?php // view filters
		...
		?>
		<select name='m'>
		<option<?php selected( $m, 0 ); ?> value='0'><?php _e('ROT Status...'); ?></option>
		</select>
	<?php } ?>
	...
	<table class="widefat <?php echo $post_type; ?> fixed content-audit" cellspacing="0">
		...
		<tr>
			...
			<?php
			foreach ($atts as $att) { ?>
				<th class="manage-column column-att column-<?php echo esc_attr(trim($att)); ?>"><?php echo trim($att); ?></th>
			<?php } ?>
	...
	<div class="alignleft actions">
	<select name="action2">
	<option value="-1" selected="selected"><?php _e('ROT Status...'); ?></option>
	</select>
...
?>
	<tr id='post-<?php echo $post->ID; ?>' class='<?php echo trim( $rowclass . ' author-' . $post_owner . ' status-' . $post->post_status ); ?> iedit' valign="top">
		<th scope="row" class="manage-column column-cb check-column"><?php echo $post->ID; ?></th>
			...
			foreach ($atts as $att) {
				$att = trim($att); ?>
				<td class="<?php echo esc_attr($att); ?>">
					<input type="checkbox" name="content_audit[attributes][<?php echo esc_attr($att); ?>]" value="1" <?php checked('1', get_post_meta($post->ID, $fields['attributes'][$att], true)); ?> />
				</td>
			<?php
			}			
			?>
